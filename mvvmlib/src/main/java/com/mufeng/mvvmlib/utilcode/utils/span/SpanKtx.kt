...
import android.text.style.QuoteSpan.STANDARD_GAP_WIDTH_PX
import android.text.style.QuoteSpan.STANDARD_STRIPE_WIDTH_PX
import android.widget.TextView
import androidx.annotation.Px
import androidx.core.content.ContextCompat
import com.mufeng.mvvmlib.utilcode.ext.fromP
import com.mufeng.mvvmlib.utilcode.utils.context
import com.mufeng.mvvmlib.utilcode.utils.span.BulletSpanKtx.Companion.STANDARD_BULLET_RADIUS
...
class SpanKtx {
    ...
    fun with(view: TextView): SpanKtx {
        this.mTextView = view
        return this
    }
    ...
    fun image(
        resId: Int,
        verticalAlignment: Int = ImageSpanKtx.ALIGN_MIDDLE,
        marginLeft: Int = 0,
        marginRight: Int = 0,
        offsetY: Int = 0,
    ...
    fun blankLine(@Px height: Int, addBlankLineAlways: Boolean = false) {
        ...
        if (mBlankLineHeight > -1) {
            mSpanBuilder.setSpan(
                BlockLineSpanKtx(mBlankLineHeight),
                start,
                end,
    ...
    private fun updateSpan() {
        ...
        mClickableSpan?.let {
            if (mTextView.movementMethod == null) mTextView.movementMethod =
        ...
        mCustomTypeface?.let {
            mSpanBuilder.setSpan(TypefaceSpanKtx(mCustomTypeface!!), start, end, mFlag)
        }
        ...
        if (mLineHeight != -1)
            mSpanBuilder.setSpan(
                LineHeightSpanKtx(mLineHeight, LineHeightSpanKtx.ALIGN_CENTER),
                start,
                end,
                mFlag
            )
        ...
        if (mQuoteColor != -1)
            mSpanBuilder.setSpan(
                if (fromP()) QuoteSpan(
                    ...
                    mQuoteStripeWidth,
                    mQuoteGapWidth
                ) else QuoteSpanKtx(mQuoteColor, mQuoteStripeWidth, mQuoteGapWidth),
                start,
                end,
                mFlag
            )
        ...
        if (mBulletColor != -1)
            mSpanBuilder.setSpan(
                if (fromP()) BulletSpanKtx(mBulletGapWidth, mBulletColor, mBulletRadius)
                else BulletSpanKtx(mBulletGapWidth, mBulletColor, mBulletRadius), start, end, mFlag
            )

        if (mBlurRadius != -1f)
            mSpanBuilder.setSpan(
                MaskFilterSpan(BlurMaskFilter(mBlurRadius, mBlurStyle)),
                start,
                end,
                mFlag
            )
    ...
    inline fun show(func: SpanKtx.() -> Unit): SpanKtx {
        func()
